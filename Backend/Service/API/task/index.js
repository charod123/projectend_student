...
const get_user_in_subdivision = async (
  {},
  { email, role, subdivision_id, division_id }
) => {
  try {
    if (role != 1) {
      return {
...
const get_task = async ({}, { email, role, subdivision_id, division_id }) => {
  try {
    ...
    let sql = pg
      .select("task.*", "tt.task_type_id", "tt.task_type_name")
      .from("task")
      .innerJoin("task_type as tt", "tt.task_type_id", "task.task_type_id");
  ...
  try {
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      if (tsk.task_type_id != 1) {
        const insert_task = await pg
          .select("up.fristname", "up.lastname")
          .from("user_master as um")
          .innerJoin("user_profile as up", "up.user_pro_id", "um.user_pro_id")
...
const update_task = async ({ values }, { email, role, subdivision_id }) => {
  try {
    ...
    for (let i = 0; i < values.length; i++) {
      const tsk = values[i];
      const insert_task = await pg("task")
        .update({
          task_title: tsk.task_title,
          task_detail: tsk.task_detail,
          start_date: moment(tsk.start_date).format("YYYY-MM-DD HH:mm:ss"),
...
const update_end_task = async (
  { end_task, task },
  { email, role, subdivision_id }
) => {
  try {
    if (!task.task_id) {
      return { code: true, status: 400, message: "ข้อมูลไม่ครบถ้วน", data: [] };
    ...
    if (role == 1) {
      sql = sql
        .update({ recommend: end_task, status: "3" })
    ...
    if (role == 2) {
      ...
      sql = sql
        .update({ detail_end_html: end_task, status: status })
        .where({ task_id: task.task_id });
    }
    const insert_task = await sql;
...
const update_status_task = async (
  { status, task_id },
  { email, role, subdivision_id }
) => {
  try {
    ...
    const insert_task = await pg("task")
      .update({ status: status == 1 ? 8 : 3, person_responsible: email })
...
const delete_file = async ({ task_id, file_name }, { email, role }) => {
  try {
    ...
    const path_read = path.join(
      __dirname,
      `../../../resources/assets/task-file/${task_id}`
    );
