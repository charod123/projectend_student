...
(function(factory, window) {
    ...
    L.Control.Ruler = L.Control.extend({
        ...
        onAdd: function(map) {
            ...
            this._container.title = 'เครื่องมือวัดระยะทาง';
        ...
        _clicked: function(e) {
            ...
            if (this._clickCount > 0 && !e.latlng.equals(this._clickedPoints[this._clickedPoints.length - 2])) {
                ...
                if (this._clickCount > 1) {
                    /// ทดสอบใส่ค่าใส่คอมม่าให้ตัวเลข
                    var numm = this._result.Distance.toFixed(this.options.lengthUnit.decimal) * 1000;
                    console.log('numm = ', numm, 'Comma(numm) =', (numm.toLocaleString()));
                    ////
                    text = '<b>' + this.options.angleUnit.label + '</b>&nbsp;' + this._result.Bearing.toFixed(this.options.angleUnit.decimal) + '&nbsp;' + this.options.angleUnit.display + '<br><b>' + this.options.lengthUnit.label + '</b>&nbsp;' + this._totalLength.toFixed(this.options.lengthUnit.decimal) + '&nbsp;' + this.options.lengthUnit.display + '<br><b>' + 'Distance:' + '</b>&nbsp;' + (this._totalLength.toFixed(this.options.lengthUnit.decimal) * 1000.00).toLocaleString() + '&nbsp;m';
                } else {
